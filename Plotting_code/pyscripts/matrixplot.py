...
fig = plt.figure()
ax1 = fig.add_subplot(111)
matrix = np.zeros((2*npt[0]+2*npt[1]+npt[2],2*npt[0]+2*npt[1]+npt[2]))
...
fig = plt.figure()
ax1 = fig.add_subplot(111)
corrmat_exp = np.nan_to_num(corrmat_exp)
...
fig = plt.figure()
ax1 = fig.add_subplot(111)
corrmat_nuc = np.nan_to_num(corrmat_nuc)
...
fig = plt.figure()
ax1 = fig.add_subplot(111)
corrmat_tot = np.nan_to_num(corrmat_tot)
...
plt.ylabel(r"$\frac{\sqrt{cov_{ii}}}{y_i}$", fontsize=35)
plt.ylim(0.01,1)
plt.xlim(0,1430)
