C *********************************************************************
C  npdf07e.f  Version 1.1                              2009/Dec/19
C
...
C    Phys. Rev. C76 (2007) 065207.
C
C  QUESTION OR COMMENT TO:
C    Masanori Hirai   [mhirai@ph.noda.tus.ac.jp]
...
C
C
C   NOTE: The returned values are multiplied by x.
C
...
C ---------------------------------------------------------------------
     ...
     +         ,PDFG(NX,NQ,ND), EMI(12,12,2)
      ...
      CHARACTER*16 FNPDF_LO(19),FNPDF_NLO(19),FGRAD_LO(19),FGRAD_NLO(19)
...
C READ GRID DATA AND CALCULATE SPLINE COEFFICIENTS. 
      ...
      IF(IORDER.EQ.1)THEN 
        OPEN(UNIT=IFILE,FILE=FNPDF_LO(ISET),STATUS='OLD')
        OPEN(UNIT=IFILE+1,FILE=FGRAD_LO(ISET),STATUS='OLD')
      ELSE IF(IORDER.EQ.2)THEN
        OPEN(UNIT=IFILE,FILE=FNPDF_NLO(ISET),STATUS='OLD')
        OPEN(UNIT=IFILE+1,FILE=FGRAD_NLO(ISET),STATUS='OLD')
      END IF
