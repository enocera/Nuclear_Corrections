...
********************************************************************************
      ...
      double precision mean_n(nexp,nset,mxpt)
      double precision np(nexp,nset,mxpt,nn) 

      character*100 wrapfile(nwrap,nexp)
      character*100 infile(nwrap,nexp,nset)
      character*100 outfile1(nexp,nset), outfile2(nexp,nset)
...
*     Define output files
      do iexp=1, nexp
         ...
         do iset=1, nset
            ...
            write(outfile1(iexp,iset),102)
     1           "res/NNnucl_",
     1           trim(nameexp(iexp)),
     1           trim(nameset(iexp,iset)), ".res"

            write(outfile2(iexp,iset),102)
     1           "res/shifts_",
     1           trim(nameexp(iexp)),
     1           trim(nameset(iexp,iset)), ".res"
      ...
      do iexp=1, nexp
         ...
         do iset=1, nset
            ...
            do i=1, npt(iexp,iset)
               ...
               mean_n(iexp,iset,i) = 0d0   

               do iwrap=1, nwrap-1

                  do irep=1, nrep(iwrap)
                     
                     in = irep + (mxrep-1)*(iwrap-1)

                     mean_n(iexp,iset,i) 
     1                    = mean_n(iexp,iset,i)
     1                    + thobs(iwrap,irep,iexp,iset,i)
     1                    / dble( nrep(1) + nrep(2) + nrep(3) )
...
*     Compute nuisance parameters
     ...
     1                    = ( thobs(iwrap,irep,iexp,iset,i) 
     1                    - mean_n(iexp,iset,i) )
     1                    / dsqrt(dble( nrep(1) + nrep(2) + nrep(3) ))
...
*     Write results on file
      do iexp=1, nexp
         ...
         do iset=1, nset
            ...
            write(*,*) nameset(iexp,iset)

            open(unit=20, status="unknown", file=outfile1(iexp,iset))
            open(unit=30, status="unknown", file=outfile2(iexp,iset))

            do i=1, npt(iexp,iset)

               write(20,103) i, ( np(iexp,iset,i,in), in=1, nn)
               write(30,104) i,  
     1              mean_n(iexp,iset,i) - mean_p(iexp,iset,i)

               write(*,*) i, mean_n(iexp,iset,i), mean_p(iexp,iset,i)
